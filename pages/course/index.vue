    ...
    <section class="container">
      ...
      <section class="c-sort-box">
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                ...
                <li v-for="subjectNested in subjectNestedList" :key="subjectNested.id">
                  <a :title="subjectNested.title" @click="searcherOne(subjectNested.id)" href="#">{{ subjectNested.title }}</a>
                </li>
              </ul>
            </dd>
          </dl>
        ...
        <div class="mt40">
          ...
          <!-- /无数据提示 结束-->
          <article class="comm-course-list" v-if="courseList.total > 0">
            <ul class="of" id="bna">
              <li v-for="item in courseList.list" :key="item.id">
                <div class="cc-l-wrap">
        ...
        <div>
          <div class="paging">
            ...
            <a :class="{ undisable: !courseList.hasPrevious }" href="#" title="前一页" @click.prevent="gotoPage(courseList.current - 1)">&lt;</a>
            <a v-for="page in courseList.pages" :key="page" :class="{current: courseList.current === page,undisable: courseList.current === page, }" :title="'第' + page + '页'" href="#" @click.prevent="gotoPage(page)">{{ page }}</a>
            <a :class="{ undisable: !courseList.hasNext }" href="#" title="后一页" @click.prevent="gotoPage(courseList.current + 1)">&gt;</a>
            <a :class="{ undisable: !courseList.hasNext }" href="#" title="末页" @click.prevent="gotoPage(courseList.pages)">末</a>
            <div class="clear" />
          </div>
        </div>
        <!-- 公共分页 结束 -->
  ...
  methods: {
    ...
    initSubject() {
      courseApi.getAllSubject().then((resp) => {
        this.subjectNestedList = resp.data.data;
    ...
    //点击某个一级分类，查询对应的二级分类
    searcherOne(subjectParentId, index) {
